...
if [ "$PORT_OK" -eq 1 ]; then
  ...
  if [ -f "$TG_ENV_FILE" ]; then
    ...
    curl -s -o /dev/null -m 10 -X POST "https://api.telegram.org/bot$TG_BOT/sendMessage" --data-urlencode "chat_id=$TG_CHAT" --data-urlencode "message_thread_id=$TELEGRAM_TOPIC_ID" --data-urlencode "text=$MSG" 2>/dev/null
